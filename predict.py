...
def _main_(args):
    ...
    #infer_model = load_model("voc_best.h5")
    ...
    if input_path[-4:] == '.mp4': # do detection on a video  
        ...
        video_out = output_path + "videos/" + input_path.split('/')[-1]
                ...
                if (i%batch_size == 0) or (i == (nb_frames-1) and len(images) > 0):
                    ...
                    for i in range(len(images)):
                        ...
                        if show_window: 
                            ...
                            cv2.imshow('video with bboxes', aux)
                            #cv2.imshow('video with bboxes', images[i])  
        ...
        for image_path in image_paths:
            ...
            cv2.imwrite(output_path + "images/" + image_path.split('/')[-1], np.uint8(image)) 
            print(type(image))
            print(image.shape)
            cv2.imshow("Detectii",image)
            cv2.waitKey(27)
